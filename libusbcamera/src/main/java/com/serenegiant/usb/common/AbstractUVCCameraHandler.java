...
public abstract class AbstractUVCCameraHandler extends Handler {
    ...
    public interface CameraCallback {
        public void onOpen(USBMonitor.UsbControlBlock ctrlBlock);
    ...
    public static abstract class SimpleCameraCallback implements CameraCallback{
        @Override
        public void onOpen(USBMonitor.UsbControlBlock ctrlBlock) {}
        @Override
        public void onClose() {}
...
//			loadShutterSound(parent);
        ...
        public AbstractUVCCameraHandler getHandler() {
            if (DEBUG) Log.i(TAG_THREAD, "getHandler:");
            synchronized (mSync) {
                if (mHandler == null)
                    try {
                        mSync.wait();
...
//		}
        ...
        public void handleOpen(final USBMonitor.UsbControlBlock ctrlBlock) {
            if (DEBUG) Log.i(TAG_THREAD, "handleOpen:");
            handleClose();
            try {
        ...
        public void handleStartPreview(final Object surface) {
            ...
            Toast.makeText(mWeakParent.get(), "预览摄像头", Toast.LENGTH_SHORT).show();
...
//                mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_YUV420SP);
                ...
                Toast.makeText(mWeakParent.get(), "预览摄像头报错了", Toast.LENGTH_SHORT).show();
                e.getStackTrace();
                try {
            ...
            if (DEBUG) Log.i(TAG_THREAD, "1 surface type="+ surface.getClass().getName());
            mUVCCamera.startPreview();
            mUVCCamera.updateCameraParams();
            synchronized (mSync) {
                if (DEBUG) Log.i(TAG_THREAD, "2 surface type="+ surface.getClass().getName());
                mIsPreviewing = true;
            }
            if (DEBUG) Log.i(TAG_THREAD, "3 surface type="+ surface.getClass().getName());
            callOnStartPreview();
        }
...
//			mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_NV21);
            ...
            if (params != null) {
                ...
                mMuxer.setOnPartialFileListener(listener);
            }
            // 启动视频编码线程
            startVideoRecord();
            // 启动音频编码线程
        ...
        private Mp4MediaMuxer.OnPartialFileListener listener = new Mp4MediaMuxer.OnPartialFileListener() {
            @Override
            public void onPartialFile(File file) {
                // 返回路径
                if (mListener != null && file != null) {
                    mListener.onRecordResult(file.getAbsolutePath());
                }
            }
        };
...
//		}
        ...
        public void run() {
            ...
            if (handler != null) {
                synchronized (mSync) {
                    if (DEBUG) Log.v(TAG_THREAD, "mSync notifyAll");
                    mHandler = handler;
                    mSync.notifyAll();
                }
                Looper.loop();
...
//				}
        ...
        private void callOnOpen(USBMonitor.UsbControlBlock ctrlBlock) {
            Log.i("www", "摄像头打开成功");
            for (final CameraCallback callback : mCallbacks) {
                try {
                    callback.onOpen(ctrlBlock);
                } catch (final Exception e) {
