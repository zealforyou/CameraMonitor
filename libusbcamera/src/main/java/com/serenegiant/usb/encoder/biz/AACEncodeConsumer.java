...
public class AACEncodeConsumer extends Thread{
    ...
    private void initAudioRecord(){
        if(DEBUG)
            Log.i(TAG,"AACEncodeConsumer-->开始采集音频");
        // 设置进程优先级
        ...
        for (final int src: AUDIO_SOURCES) {
            try {
                mAudioRecord = new AudioRecord(src,
                        SAMPLE_RATE, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_8BIT, bufferSize);
    ...
    private void initMediaCodec(){
        if(DEBUG)
            Log.i(TAG,"AACEncodeConsumer-->开始编码音频");
    ...
    private void stopAudioRecord() {
        if(DEBUG)
            Log.i(TAG,"AACEncodeConsumer-->停止采集音频");
        if(mAudioRecord != null){
            mAudioRecord.stop();
            mAudioRecord.release();
            mAudioRecord = null;
    ...
    private void stopMediaCodec() {
        if(DEBUG)
            Log.i(TAG,"AACEncodeConsumer-->停止编码音频");
