...
public class Mp4MediaMuxer {
    ...
    private OnPartialFileListener onPartialFileListener;
    ...
    public OnPartialFileListener getOnPartialFileListener() {
        return onPartialFileListener;
    }

    public void setOnPartialFileListener(OnPartialFileListener onPartialFileListener) {
        this.onPartialFileListener = onPartialFileListener;
    }
...
//            if (VERBOSE)
//                Log.i(TAG, String.format("sent %s [" + bufferInfo.size + "] with timestamp:[%d] to muxer", isVideo ? "video" : "audio", bufferInfo.presentationTimeUs / 1000));
        }
...
//                if (VERBOSE)
                ...
                if (onPartialFileListener != null) {
                    try {
                        onPartialFileListener.onPartialFile(new File(getCurrentPath()));
                    } catch (Exception e) {
                    }
                }
                try {
    ...
    private String genVideoFilePath() {
        if (durationMillis != 0) {
            return mDir + "-" + index++ + ".mp4-tmp";
        } else {
            return mDir + ".mp4-tmp";
        }
    }
    ...
    public static interface OnPartialFileListener {
