...
public class H264EncodeConsumer extends Thread {
    ...
    public void setRawYuv(byte[] yuvData, int width, int height) {
        ...
        frameTimeSum += System.currentTimeMillis() - lastPush;
        if (frameTimeSum > 1000 / 20) {
            frameTimeSum = frameTimeSum- 1000 / 20;
            // 将数据写入编码器
//            nv12ToNV21(yuvData, mWidth, mHeight)
            feedMediaCodecData(yuvData);
            FpsUtils.INSTANCE.compute();
        }
        lastPush = System.currentTimeMillis();
    }
    ...
    private void startMediaCodec() {
        ...
        format.setInteger(MediaFormat.KEY_BIT_RATE, (int) Config.bit_rate);
