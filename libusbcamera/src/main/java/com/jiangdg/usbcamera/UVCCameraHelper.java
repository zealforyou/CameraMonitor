...
public class UVCCameraHelper {
    ...
    private HashMap<String, USBMonitor.UsbControlBlock> cameraMap = new HashMap<>();
    ...
    public void initUSBMonitor(Activity activity, CameraViewInterface cameraView, final OnMyDevConnectListener listener) {
        ...
        mUSBMonitor = new USBMonitor(activity.getApplicationContext(), new USBMonitor.OnDeviceConnectListener() {
            ...
            public void onConnect(final UsbDevice device, USBMonitor.UsbControlBlock ctrlBlock, boolean createNew) {
                ...
                cameraMap.put(device.getDeviceName(), ctrlBlock);
                Log.i("www", "device onConnect");
        ...
        createUVCCamera();
        addCallback(new AbstractUVCCameraHandler.SimpleCameraCallback() {
            @Override
            public void onOpen(USBMonitor.UsbControlBlock ctrlBlock) {
                super.onOpen(ctrlBlock);
...
//            throw new NullPointerException("mUSBMonitor ="+mUSBMonitor+"deviceFilters=;"+deviceFilters);
    ...
    public void addCallback(final AbstractUVCCameraHandler.CameraCallback callback) {
        if (mCameraHandler == null)
            return;
        mCameraHandler.addCallback(callback);
    }

    public void removeCallback(final AbstractUVCCameraHandler.CameraCallback callback) {
        if (mCameraHandler == null)
            return;
        mCameraHandler.removeCallback(callback);
    }
