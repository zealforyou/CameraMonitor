...
public abstract class CameraGLRendererBase implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized(this) {
            ...
            if(texListener != null) {
                ...
                Log.i(LOGTAG, "texCamera(OES) -> screen");
    ...
    private void initSurfaceTexture() {
        Log.i(LOGTAG, "initSurfaceTexture");
    ...
    private void deleteSurfaceTexture() {
        Log.i(LOGTAG, "deleteSurfaceTexture");
        if(mSTexture != null) {
            mSTexture.release();
            mSTexture = null;
            deleteTex(texCamera);
    ...
    private static int loadShader(String vss, String fss) {
        Log.i("CameraGLRendererBase", "loadShader");
        ...
        Log.i("CameraGLRendererBase", "Shader program is built OK");
    ...
    {
        Log.i(LOGTAG, "deleteFBO("+mFBOWidth+"x"+mFBOHeight+")");
    ...
    {
        Log.i(LOGTAG, "initFBO("+width+"x"+height+")");
        ...
        Log.i(LOGTAG, "initFBO error status: " + GLES20.glGetError());
    ...
    public synchronized void enableView() {
        Log.i(LOGTAG, "enableView");
        mEnabled = true;
        updateState();
    }

    public synchronized void disableView() {
        Log.i(LOGTAG, "disableView");
        mEnabled = false;
        updateState();
    }

    protected void updateState() {
        Log.i(LOGTAG, "updateState");
        Log.i(LOGTAG, "mEnabled="+mEnabled+", mHaveSurface="+mHaveSurface);
        boolean willStart = mEnabled && mHaveSurface && mView.getVisibility() == View.VISIBLE;
        if (willStart != mIsStarted) {
            if(willStart) doStart();
            else doStop();
        } else {
            Log.i(LOGTAG, "keeping State unchanged");
        }
        Log.i(LOGTAG, "updateState end");
    }

    protected synchronized void doStart() {
        Log.i(LOGTAG, "doStart");
        initSurfaceTexture();
