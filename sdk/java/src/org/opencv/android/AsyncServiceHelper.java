...
class AsyncServiceHelper
{
    ...
    protected static void InstallService(final Context AppContext, final LoaderCallbackInterface Callback)
    {
        ...
        {
                Log.i(TAG, "Request new service installation");
                ...
                public void install() {
                    Log.i(TAG, "Trying to install OpenCV Manager via Google Play");

                    boolean result = InstallServiceQuiet(AppContext);
                    if (result)
                    {
                        mServiceInstallationProgress = true;
                        Log.i(TAG, "Package installation started");
                    }
                    else
                    {
                        Log.i(TAG, "OpenCV package was not installed!");
                        int Status = LoaderCallbackInterface.MARKET_ERROR;
                        Log.i(TAG, "Init finished with status " + Status);
                        Log.i(TAG, "Unbind from service");
                        Log.i(TAG, "Calling using callback");
                ...
                public void cancel()
                {
                    Log.i(TAG, "OpenCV library installation was canceled");
        ...
        {
            Log.i(TAG, "Waiting current installation process");
            InstallCallbackInterface WaitQuery = new InstallCallbackInterface() {
                ...
                public void cancel()
                {
                    Log.i(TAG, "Waiting for OpenCV canceled by user");
    ...
    protected ServiceConnection mServiceConnection = new ServiceConnection()
    {
        ...
        {
            Log.i(TAG, "Service connection created");
            mEngineService = OpenCVEngineInterface.Stub.asInterface(service);
            if (null == mEngineService)
            {
                Log.i(TAG, "OpenCV Manager Service connection fails. May be service was not installed?");
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        Log.i(TAG, "Init finished with status " + LoaderCallbackInterface.INCOMPATIBLE_MANAGER_VERSION);
                        Log.i(TAG, "Unbind from service");
                    ...
                    Log.i(TAG, "Trying to get library path");
                    ...
                    if ((null == path) || (path.length() == 0))
                    {
                        if (!mLibraryInstallationProgress)
                        {
                            InstallCallbackInterface InstallQuery = new InstallCallbackInterface() {
                                ...
                                public void install() {
                                    Log.i(TAG, "Trying to install OpenCV lib via Google Play");
                                    try
                                    {
                                        ...
                                        {
                                            Log.i(TAG, "OpenCV package was not installed!");
                                            Log.i(TAG, "Init finished with status " + LoaderCallbackInterface.MARKET_ERROR);
                                            Log.i(TAG, "Unbind from service");
                                            mAppContext.unbindService(mServiceConnection);
                                            Log.i(TAG, "Calling using callback");
                                        ...
                                        e.printStackTrace();;
                                        Log.i(TAG, "Init finished with status " + LoaderCallbackInterface.INIT_FAILED);
                                        Log.i(TAG, "Unbind from service");
                                        mAppContext.unbindService(mServiceConnection);
                                        Log.i(TAG, "Calling using callback");
                                ...
                                public void cancel() {
                                    Log.i(TAG, "OpenCV library installation was canceled");
                                    Log.i(TAG, "Init finished with status " + LoaderCallbackInterface.INSTALL_CANCELED);
                                    Log.i(TAG, "Unbind from service");
                                    mAppContext.unbindService(mServiceConnection);
                                    Log.i(TAG, "Calling using callback");
                        ...
                        else
                        {
                            InstallCallbackInterface WaitQuery = new InstallCallbackInterface() {
                                ...
                                public void wait_install() {
                                    Log.i(TAG, "Waiting for current installation");
                                    try
                                    {
                                        if (!mEngineService.installVersion(mOpenCVersion))
                                        {
                                        ...
                                        {
                                            Log.i(TAG, "Waiting for package installation");
                                        }
    ...
    private boolean loadLibrary(String AbsPath)
    {
        ...
        Log.i(TAG, "Trying to load library " + AbsPath);
        try
        {
            System.load(AbsPath);
            Log.i(TAG, "OpenCV libs init was ok!");
        }
        catch(UnsatisfiedLinkError e)
        {
            Log.i(TAG, "Cannot load library \"" + AbsPath + "\"");
            e.printStackTrace();
            result = false;
        }
    ...
    private boolean initOpenCVLibs(String Path, String Libs)
    {
        ...
        {
            Log.i(TAG, "Library path \"" + Path + "\" is empty");
            return false;
        }
    }
}
